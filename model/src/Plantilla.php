...
class Plantilla extends Mysql
{
        ...
        public function getGenerarPlantilla($request)
        {
                ...
                try {
                        ...
                        if ($proyecto_generales) {
                                ...
                                if ($proyecto_generales->fecha_fin) $var["fecha_fin"] = $proyecto_generales->fecha_fin;
                                if ($proyecto_generales->costo_directo) $var["costo_directo"] = $proyecto_generales->costo_directo;
        ...
        private function getInsertSubcategoria()
        {
                ...
                if ($subcategorias_proyecto_general) {
                        foreach ($subcategorias_proyecto_general as  $value) {
                                ...
                                if ($value->subcategorias_master_id) $var['subcategorias_master_id'] = $value->subcategorias_master_id;
        ...
        private function getInsertTitulos()
        {
                ...
                if ($titulos_proyecto) {
                        foreach ($titulos_proyecto as  $value) {
                                ...
                                if ($value->master_title_id) $var['master_title_id'] = $value->master_title_id;
        ...
        private function getInsertPresupuesto()
        {
                ...
                if ($presupuestos) {
                        ...
                        foreach ($object as $value) {
                                ...
                                if ($value->metrado) $var["metrado"] = $value->metrado;
                                if ($value->cu) $var["cu"] = $value->cu;
                                if ($value->mo) $var["mo"] = $value->mo;
                                if ($value->mt) $var["mt"] = $value->mt;
                                if ($value->eq) $var["eq"] = $value->eq;
                                if ($value->sc) $var["sc"] = $value->sc;
                                if ($value->sp) $var["sp"] = $value->sp;

                                if ($value->unidad_medidas_id) $var["unidad_medidas_id"] = $value->unidad_medidas_id;
                                if ($value->presupuestos_title_id) $var["presupuestos_title_id"] = $this->titulos[$value->presupuestos_title_id];
                                if ($value->partidas_id) $var["partidas_id"] = $this->partidas_proyecto[$value->partidas_id];

                                if ($value->subpresupuestos_id) $var["subpresupuestos_id"] = $this->subpresupuestos[$value->subpresupuestos_id];
        ...
        private function setVerificarPresupuesto($searchedValue, $dataPresupuesto, $id)
        {
                ...
                if (count($object)) {
                        foreach ($object as $value) {
                                ...
                                $insert = self::insert("presupuestos", $var);
                                if ($value->type_item == 3) $this->partidas[$value->id] = $insert["lastInsertId"];
        ...
        private function getInsertGastosGenerales()
        {
                ...
                if ($gastos_generales) {
                        ...
                        foreach ($gastos_generales as  $value) {
                                ...
                                if ($value->duracion) $var["duracion"] = $value->duracion;
                                if ($value->cantidad) $var["cantidad"] = $value->cantidad;
                                if ($value->porcentaje_partida) $var["porcentaje_partida"] = $value->porcentaje_partida;
                                if ($value->precio) $var["precio"] = $value->precio;
                                if ($value->parcial) $var["parcial"] = $value->parcial;
                                if ($value->gastos_generales_id) $var["gastos_generales_id"] = $ggenerales_id[$value->gastos_generales_id];
                                $insert = self::insert("gastos_generales", $var);
                                if (!$value->gastos_generales_id) $ggenerales_id[$value->id] = $insert["lastInsertId"];
                        }
                }
        }
        ...
        private function getInsertPartidasProyecto()
        {
                ...
                if ($partidas_proyecto) {
                        foreach ($partidas_proyecto as  $value) {
                                ...
                                if ($value->unidad_medidas_id) $var["unidad_medidas_id"] = $value->unidad_medidas_id;
                                if ($value->master_partida_id) $var["master_partida_id"] = $value->master_partida_id;
                                if ($value->proyectos_generales_id) $var["proyectos_generales_id"] = $this->_newId;
        ...
        private function getInsertInsumosProyecto()
        {
                ...
                if ($insumos_proyecto) {
                        foreach ($insumos_proyecto as  $value) {
                                ...
                                if ($value->iu) $var["iu"] = $value->iu;
                                if ($value->precio) $var["precio"] = $value->precio;
                                if ($value->master_insumo_id) $var["master_insumo_id"] = $value->master_insumo_id;
        ...
        private function getInsertApus()
        {
                ...
                $apus_partida_presupuestos =  self::fetchAllObj($sql);
                                ...
                                if ($value->cuadrilla) $var["cuadrilla"] = $value->cuadrilla;
                                if ($value->cantidad) $var["cantidad"] = number_format($value->cantidad, 4, '.', '');
                                if ($value->precio) $var["precio"] = number_format($value->precio, 2, '.', '');
                                if ($value->partida_id) $var["partida_id"] = $this->partidas_proyecto[$value->partida_id];
                                if ($value->subpartida_id) $var["subpartida_id"] = $this->subpartidas[$value->subpartida_id];
                                if ($value->iu) $var["iu"] = $value->iu;
                                if ($value->monomio) $var["monomio"] = $value->monomio;
                                $insert = self::insert("apus_partida_presupuestos", $var);
                                if ($value->partida_id) $this->subpartidas[$value->id] = $insert["lastInsertId"];
                        }
                }
        }
        ...
        private function getInsertMetrados()
        {
                ...
                if ($metrado_partida_presupuestos) {
                        foreach ($metrado_partida_presupuestos as  $value) {
                                ...
                                if ($value->position) $var["position"] = $value->position;
                                if ($value->metrado_largo) $var["metrado_largo"] = $value->metrado_largo;
                                if ($value->metrado_alto) $var["metrado_alto"] = $value->metrado_alto;
                                if ($value->metrado_ancho) $var["metrado_ancho"] = $value->metrado_ancho;
                                if ($value->metrado_area) $var["metrado_area"] = $value->metrado_area;
                                if ($value->metrado_volumen) $var["metrado_volumen"] = $value->metrado_volumen;
                                if ($value->metrado_cantidad) $var["metrado_cantidad"] = $value->metrado_cantidad;
                                if ($value->metrado_nro_elemto) $var["metrado_nro_elemto"] = $value->metrado_nro_elemto;
                                if ($value->metrado_factor) $var["metrado_factor"] = $value->metrado_factor;
                                if ($value->img) $var["img"] = $value->img;
                                if ($value->is_information) $var["is_information"] = $value->is_information;
        ...
        private function getInsertFormulasPolinomica()
        {
                ...
                if ($formula_polinomica) {
                        foreach ($formula_polinomica as  $value) {
                                ...
                                if ($value->monomio) $var["monomio"] = $value->monomio;
                                if ($value->simbolo) $var["simbolo"] = $value->simbolo;
                                if ($value->subpresupuesto_id) $var["subpresupuesto_id"] = $value->subpresupuesto_id;
        ...
        private function getInsertPiePesupuesto()
        {
                ...
                if ($proyecto_pie_presupuesto) {
                        foreach ($proyecto_pie_presupuesto as  $value) {
                                ...
                                if ($value->percentage) $var["percentage"] = $value->percentage;
                                if ($value->active) $var["active"] = $value->active;
                                if ($value->type_percentage) $var["type_percentage"] = $value->type_percentage;
                                if ($value->pie_presupuesto_id) $var["pie_presupuesto_id"] = $value->pie_presupuesto_id;
        ...
        private function getInsertPiePresupuestoGrupo()
        {
                ...
                if ($pie_presupuesto_grupo) {
                        foreach ($pie_presupuesto_grupo as  $value) {
                                ...
                                if ($value->iu) $var["iu"] = $value->iu;
                                if ($value->monomio) $var["monomio"] = $value->monomio;
                                if ($value->subpresupuestos_id) $var["subpresupuestos_id"] = $value->subpresupuestos_id;
