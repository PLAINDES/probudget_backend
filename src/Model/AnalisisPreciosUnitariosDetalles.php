...
//use App\Model\AnalisisPreciosUnitariosDetalles;
...
class AnalisisPreciosUnitariosDetalles extends Mysql
{
    ...
    public function getAnalisisPreciosUnitariosId()
    {
        return $this->_analisis_precios_unitarios_id;
    }
    private $_monto_parcial_ppto;
    public function getMontoParcialPpto()
    {
    ...
    public function getSave()
    {
        try {
            if ($this->_cuadrilla == 0 && $this->_cantidad == 0 && $this->_precio == 0) {
                $this->_values["precio"] = number_format($this->getInsumosId(), 2, '.', '');
