...
class User extends Mysql
{
    ...
    public function signUpdate($id, $email, $accept_policies, $domain, $oauth_provider, $password_rand = null)
    {
        $resp = array();
        try {
            $password = FG::crypt($password_rand);
    ...
    public function restorePassword($request)
    {
        ...
        try {
            ...
            if ($result->success) {
                $id = $result->data->id;
                $password =  FG::crypt($request->newpassword);
                $code_password = null;
      ...
      // DEBUG: log entrada completa
        ...
        try {
            ...
            if ($isMultiple) {
                ...
                if ($actualizados > 0) {
                    $mensaje .= ", se actualizaron {$actualizados} asignación(es)";
                }
                if ($errores > 0) {
                    $mensaje .= ". {$errores} proyecto(s) presentaron errores";
                }
